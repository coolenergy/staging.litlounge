...
import './index.less';
import { beforeAvatarUpload, beforeHeaderUpload } from '@lib/file';
...
class PerformerProfilePage extends PureComponent<P> {
  ...
  onUploadedHeader(data: any) {
    const { performer, updatePerformerProfileSuccess: dispatchUpdatePerformerProfileSuccess, setupdatingPerformerProfile: dispatchSetUpdating } = this.props;
    dispatchSetUpdating();
    dispatchUpdatePerformerProfileSuccess({
      ...performer,
      header: data.response.data.url
    });
  }
      ...
      <>
        ...
        <div className="performer-profile-page">
          ...
          >
            ...
            <Tabs.TabPane tab="Profile Header" key="profile-header">
              <Form.Item label="Profile Header">
                <ImageUpload
                  options={{ fieldName: 'header' }}
                  imageUrl={performer?.header}
                  uploadUrl={performerService.getHeaderUploadUrl()}
                  headers={uploadHeaders}
                  beforeUpload={beforeHeaderUpload}
                  onUploaded={this.onUploadedHeader.bind(this)}
                />
                <div>Image size limit: 3MB. Accepted formats: JPEG, JPG and PNG</div>
              </Form.Item>
            </Tabs.TabPane>
